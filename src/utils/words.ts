...
export const createExplanation = createServerFn({ method: "POST" })
  .validator((word: string) => z.string().parse(word))
...
export const getRandomWord = createServerFn({ method: "GET" })
  .validator((opts: { notIn?: string[]; limit?: number }) => z.object({
  ...
  .handler(async ({ data }) => {
      ...
      let query = db.select().from(words).orderBy(sql`RANDOM()`).limit(data.limit);
